...
def gerar_codigo_orquestrador(caso_de_uso: str, plano: PlanoArquiteturaMultiAgentes) -> CodigoOrquestrador:
    plano_json = json.dumps(plano.model_dump(), ensure_ascii=False, indent=2)
...
- Se precisar de schemas, use pydantic.BaseModel
    ...
    if not caso_de_uso.strip():
        ...
        with st.spinner("Gerando o código do orquestrador com base na recomendação final..."):
            try:
                ...
                valido, erro = validar_codigo_python(codigo.codigo_python)
                if not valido:
                    st.error("O código gerado veio com erro de sintaxe.")
                    st.code(codigo.codigo_python, language="python")
                    st.error(f"Detalhe: {erro}")
                    st.stop()
